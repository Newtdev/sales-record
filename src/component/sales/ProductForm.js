...
const ProductForm = props => {
  const renderInput = ({ input, label, meta }) => {
    const renderErrors = ({ error, touched }) => {
      if (error && touched) {
        return (
          <p className='text-red-500 text-xs  italic pt-1'>{`${error}`}</p>
        );
      }
    };

    return (
      <div className={`py-${props.padding}`}>
        <label
          className={`block text-sm font-medium ${props.text ||
        ...
        <input
          name={input.name}
          {...input}
          className={`shadow border border-gray-100 ${props.color} rounded-sm w-full py-1 px-2 capitalize text-gray-800 focus:outline-none focus:shadow-outline`}
        />
        {renderErrors(meta)}
      </div>
    );
  ...
  return (
    <div className='w-full h-screen flex justify-center sm:justify-start items-center '>
      <div className='h-full w-80  sm:w-1/2 pb-5'>
        <form
          className=' w-full ml-5 sm:pr-20 pb-5'
